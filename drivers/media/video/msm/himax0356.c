...
static int himax0356_probe_init_sensor(const struct msm_camera_sensor_info *data)
{
	...
	rc = gpio_request(data->sensor_reset, "himax0356");
	if (!rc)
	...
	else
	{
		...
		rc=gpio_request(data->sensor_reset, "himax0356");
		gpio_direction_output(data->sensor_reset, 0);
		printk("himax0356_probe_init_sensor gpio_direction_output(data->sensor_reset, 0)rc =%d",rc);
	}
		...
		struct msm_sensor_ctrl *s)
{
	...
	CDBG("himax0356_sensor_probe start\n");
	if (rc < 0 || himax0356_client == NULL) {
		rc = -ENOTSUPP;
		goto probe_done;
	}
    CDBG("i2c_add_driver  rc=%d s\n",rc);
	/* enable mclk first */
	msm_camio_clk_rate_set(HIMAX0356_DEFAULT_CLOCK_RATE);
	mdelay(20);
	...
	rc = himax0356_probe_init_sensor(info);
	    ...
	    CDBG("himax0356_probe_init_sensor  rc=%d s\n",rc);
	if (rc < 0) {
